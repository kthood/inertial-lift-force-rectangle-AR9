...
%  inertial focusing of particles in a microchannel with aspect ratio AR=9
%  assuming no noise or particle-particle interactions
%  creates a .gif animation
%
%% AUTHOR
%  Kaitlyn Hood 2016 - kaitlyn.t.hood@gmail.com
%%%

% parameters:
Re = 36;                    % Reynolds number
W = 405e-6; H = 45e-6;      % height and width of channel (m)
...
for j=1:size(xs,1)
    for l=1:size(xs,2)
        [forcex(j,l),forcey(j,l)]=getLiftForce_AR9(xs(j,l),ys(j,l),alpha,U,rho,L);
    end
end
...
%         try
%             drag = 6*pi*mu*alpha;
%             [utemp,vtemp] = getLiftForce_AR9(x(k,j-1),y(k,j-1),alpha,U,rho,L);
%             V = [utemp; vtemp]/drag;
%             [v1,v2] = getLiftImage_AR9(x(k,j-1),y(k,j-1),alpha,L);
%             v1 = alpha*v1*abs(V(1)); 
%             v2 = alpha*v2*abs(V(2));
%             vel = V + v1 + v2;
%             u(k) = vel(1); 
...
% Advect particles using the lift force
% fast method
load inertial_constants_AR2_n901_04-18-2016.mat
x=zeros(m,N); y=zeros(m,N);
u=zeros(m,1); v=zeros(m,1);
x(:,1)=r(:,1); y(:,1)=r(:,2); 
for j=2:N;
...
figure('Units','pixels','Position',[100,100,600,150])
clf
